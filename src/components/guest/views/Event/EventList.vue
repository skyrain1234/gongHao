<!-- src/components/guest/views/news/EventList.vue -->
<template>
	<div>
		<!-- =============================
         清單區（卡片列表）
    ============================== -->
		<div class="row g-4 mb-3 mt-2">
				...
				<div class="card h-100 news-card box_roller border-0 shadow-sm">
					...
					<!-- <div class="roller">
						<router-link
							:to="{
								name: 'news',
								params: { lang: route.params.lang, newsId: eventItem.id },
							}"
					...
					<div class="news-media">
						<img
							:src="getStorageFileUrl(eventItem.thumbnail_url)"
							:alt="eventItem.title"
							class="w-100 h-100"
							v-if="eventItem.thumbnail_url"
						/>
						<img
					...
					<div class="card-body d-flex flex-column news-card-body">
						...
						>
							...
							<p class="news-date mb-0">
								{{ eventItem.publish_date }}
							</p>
							<!-- 查看內容 -->
							<router-link
								:to="{
									name: 'event',
									params: { lang: route.params.lang, eventId: eventItem.id },
								}"
								class="news-more"
							>
								{{$t('common.more')}}
			...
			<!-- 沒資料 -->
			<div v-else-if="!paginatedEvent.length" class="col-12 text-center py-5">
				{{ $t("common.no-data") }}
			</div>
		</div>
    ...
    ============================== -->
		<div v-if="!isLoading" class="mx-auto pb-3">
			<Pagination
				...
				:limit="pageLimit"
				:total-items="filteredEvent.length"
				:window-size="3"
				:show-info="true"
				:show-jump="true"
			/>
...
const searchByKeyword = async (keyword) => {
	...
	try {
		...
		if (!keyword) {
			...
			// 有關鍵字 → 呼叫搜尋 API
			const result = await getEventSearch(keyword);
			eventList.value = result;
		}
	} catch (err) {
...
const filteredEvent = computed(() => {
	if (!props.selectedClassId) return eventList.value;
 ...
 * 計算屬性：分頁
 * ============================ */
const pageLimit = Number(appConfig.home.eventLimit ?? 10);

const totalPages = computed(() =>
	Math.max(1, Math.ceil(filteredEvent.value.length / pageLimit))
);

const paginatedEvent = computed(() => {
	const start = (currentPage.value - 1) * pageLimit;
	return filteredEvent.value.slice(start, start + pageLimit);
});
